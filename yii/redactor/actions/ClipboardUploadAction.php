...
class ClipboardUploadAction extends \yii\base\Action {
	...
	public function run() {
		if ($this->_contentType && $this->_data) {
			if (file_put_contents($this->getPath(),
			  base64_decode($this->_data))) {
				echo Json::encode(array(
				  'filelink' => $this->getUrl(),
	...
	protected function getExtensionName() {
		...
		return (array_search($this->_contentType,
			$mimeTypes) !== false) ? array_search($this->_contentType,
		  $mimeTypes) : 'png';
	}

	protected function getFilename() {
		if (!$this->_filename) {
			$this->_filename = substr(uniqid(md5(rand()), true), 0,
				10) . '.' . $this->getExtensionName();
		}
	...
	protected function getUrl() {
		return str_replace(DIRECTORY_SEPARATOR, '/',
		  str_replace(Yii::getAlias('@webroot'), '', $this->getPath()));
	}
